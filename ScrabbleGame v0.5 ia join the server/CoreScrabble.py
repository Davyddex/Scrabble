...
# {key: Objet, ...}
Objets_later_use = {}    # Store tkinter objet that are initialise in a function but need to be use in a other one
...
# index 0  -> player 1  {"ClientName":"Solo/client adress/AI{difficulty}", "UserName":"MyName", "Turn": True/False, "SpaceInHand":7-space_left_in_hand()}
...
def creatTemporyText(plan, path, txt, x, y):
    """Creat a text box in the plan at coord x,y if the function is call
    a second time the previous message is deleted"""
    if path in info_box:
...
def creatserverLobbyCanvas():
    ...
    AddAIButton = tk.Button(ServerLobbyCanvas, text="  Add AI  ", font=("", "11", "bold"),
                           command=Add_AI_lobby)
    Objets_later_use["B_AddAI"] = AddAIButton
    ...
    if nbplayer != max and SaveName[0] == Players_info[0]["UserName"]:
        AddAIButton.pack(anchor="e", padx="150", pady="300")

    if nbplayer == max:
        # if there are enough player
...
def receive_data():
    ...
    while Online[0]:
        ...
        try:
            ...
            pass
        # exist the while <=> deconnection
    deco = objetHeader(f"c.lobby.{DATA_DISCONNECT}").encode(DECFOR)
    client.send(deco)


def remove_bonus(lorc, nb, start, end):
    """Remove the bonus at a given word position"""
    if lorc == "l":
...
def Add_AI_lobby():
    AddAIButton = Objets_later_use["B_AddAI"]
    AddAIButton.destroy()
    client.send((objetHeader("c.lobby.add_ai")).encode(DECFOR))
    ...
    for new_playerdata in Listinfoserver:
        for player_index in range(0, len(Players_info)):
            if new_playerdata["UserName"] == Players_info[player_index]["UserName"]:
                ...
                Players_info[player_index]["Score"] = new_playerdata["Score"]
    return modif
...
def add_word_in_line_or_column():
    ...
    Thus returning the information about how the letter place by the player are"""
...
def always_word_on(lorc, nb, start, end):
    """Return all the information (word) of a line/column
    in range of the start and the end even None letter"""
    word = ""
    if lorc == "l":
        for column in range(start, end + 1):
            letter = grid_list[nb, column][2]  # give "" if noting, " " for a joker, "letter" for a letter
                ...
                if lorc == "c" or lorc == "l" or lorc == "l/c":
                    ...
                    if word:
                        ...
                        if valid_word(word) or valid_word_joker(word, list_pos_joker):
                            ...
                            if not find_False_in_list(list_of_auxiliary_word_bonus) or not is_lock(7, 7):
                                total += value_of_a_word(lorc, nb, newstar, newend)


                                for (lorc, nb, start, end) in list_of_auxiliary_word_bonus:
...
def getnonelockletter():
    """Return all the letter that are not lock (thus place by the player)"""
    L = []
    for row, column in List_objet_in_grid:
...
def sort_Players():
    ...
    size = len(List_of_Player)
    for ind in range(0, size):
        s, n = List_of_Player[0]
        for score, name in List_of_Player:
            if score > s:
                s = score
                n = name
        List_of_Player.remove((s, n))
        L.append((s, n))
