...
active_cmd = {"/addinpool","/putletteringrid","/removeinpool","/removeingrid","/addinhand","/removeinhand","/setscore"
              ,"/end","/kick"}
            ...
            if inp[0] == "/":
                ...
                if inp == "/help":
                    ...
                    if "A" <= cmd_input[1] <= "Z" and (0 <= int(cmd_input[2]) <= 14 and 0 <= int(cmd_input[3]) <= 14) and GameStart[0]:
                        send_to_list(client_connected, f"s.game.play.l/{int(cmd_input[2])}/{int(cmd_input[3])}/{int(cmd_input[3])}/{int(cmd_input[1])}")
